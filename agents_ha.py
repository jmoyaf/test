...
zabbix_conn = ZabbixConn("http://10.26.192.48", "zabbix.api","zabbix")
zabbix_conn.connect()
...
for u in servers:
  ...
  if history[1][0]=='1' and history[1][1]=='1':
...
for num in range(0,number_servers_get_up):
  ...
  command = "/opt/zabbix/server/bin/zabbix_get -s %s -k agent.version" % list_ip_possible_servers[num]
  os.system(command)